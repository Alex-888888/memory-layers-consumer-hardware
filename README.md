...
- **No emergent entity-level safety, and no *internal* fix (v0.3.4 / v0.4.1).** On non-stored entities the model **confidently fabricates plausible, format-correct wrong values**; it does not abstain (`docs/SAFETY_EVAL.md`). We then tested whether an internal open-set detector could add abstention: across routing geometry, value-space geometry (Tyler) and semantic entropy, the signal is **at or near chance (AUC 0.50 / 0.50 / 0.66)** — the fabrications are confident and self-consistent, so there is no uncertainty to read. Reliable entity-level abstention therefore requires an **external** membership/retrieval check — see [`docs/OPENSET_MP_TYLER.md`](docs/OPENSET_MP_TYLER.md).
